"""
Copied from Biapy U-NeXt-V2 model.
...
from sknext.model.blocks import UpConvNeXtBlock_V2, ConvNeXtBlock_V2, prepare_activation_layers, init_weights
from torchvision.ops.misc import Permute
from sknext.model.heads import ProjectionHead
...
class U_NeXt_V2(nn.Module):
    ...
    def __init__(
        ...
        if contrast and len(output_channels) > 2:
            raise ValueError(
                "If 'contrast' is True, 'output_channels' can only have two values at max: one for the main output and one for the class.")
        ...
        if self.explicit_activations:
            assert len(head_activations) == sum(
                output_channels), "If 'explicit_activations' is True, 'head_activations' needs to "
            "have the same number of values as 'output_channels'"
            self.head_activations, self.class_head_activations = prepare_activation_layers(head_activations,
                                                                                           output_channel_info,
                                                                                           output_channels)
