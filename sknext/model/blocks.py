...
class GRN(nn.Module):
    ...
    def __init__(self, dim,ndim:int):
        """
        ...
        super().__init__()
        self.ndim = ndim
        self.gamma = nn.Parameter(torch.zeros(*([1] * (ndim + 1)), dim))
        self.beta = nn.Parameter(torch.zeros(*([1] * (ndim + 1)), dim))
    ...
    def forward(self, x):
        ...
        spatial_dims = tuple(range(1, x.ndim - 1))  # 2D: (1,2), 3D: (1,2,3)
...
def get_norm_3d(norm: str, out_channels: int, bn_momentum: float = 0.1) -> nn.Module:
    ...
    if norm in ["bn", "sync_bn"]:
        ...
        return selected_norm(num_channels=out_channels, num_groups=8)
    else:
...
def get_norm_2d(norm: str, out_channels: int, bn_momentum: float = 0.1) -> nn.Module:
    ...
    if norm in ["bn", "sync_bn"]:
        ...
        return selected_norm(num_channels=out_channels, num_groups=16)
    else:
        return selected_norm()
