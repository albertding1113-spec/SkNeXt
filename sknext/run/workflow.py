...
class Segmentation_Workflow(Base_Workflow):
    def __init__(self, cfg, device:torch.device, job_id:int):
        ...
        print(f"{time_str()} Init Segmentation.")
    ...
    def load_checkpoint(self):
        ...
        self.model.to(self.device)
        print(f"{time_str()} Model checkpoint loaded from: {self.checkpoint_file}")
    ...
    def save_checkpoint(self):
        ...
        print(f"{time_str()} Model checkpoint saved to: {self.checkpoint_file}")

    def train(self):
        print(f"{time_str()} preparing training data")
        self.save_patch_as_zarr()
        print(f"{time_str()} preparing data loader")
        ...
        print(f"{time_str()} preparing model")
        self.set_model()
        ...
        print(f"{time_str()} preparing optimizer and scheduler")
        self.set_optimizer_scheduler()
        print(f"{time_str()} preparing loss function and metrics")
        ...
        for epoch in range(self.epochs):
            print(f"{time_str()} [EPOCH{epoch+1:04d}]")
            self.model.train()
            epoch_train_loss = 0
            ...
            for step in range(self.train_steps_per_epoch):
                ...
                if (step+1)%math.ceil(self.train_steps_per_epoch/20) == 0:
                    ...
                    print(f"{time_str()} [EPOCH{epoch+1:04d}] [TRAIN] [{epoch_train_patches:06d}/{self.train_patches:06d}], loss: {step_loss: 6f}, {metric_str}, lr: {self.optimizer.param_groups[0]['lr']:.6f}")
                # backward
                step_loss.backward()
                self.optimizer.step()
                self.scheduler.step()
            ...
            print(f"{time_str()} [EPOCH{epoch+1:04d}] [TRAIN] loss: {epoch_train_loss:.6f}, {metric_str}")
            ...
            with torch.no_grad():
                for step in range(self.val_steps_per_epoch):
                    ...
                    if (step+1) % math.ceil(self.val_steps_per_epoch / 10) == 0:
                        metric_str = " ".join(f"{key} channel: {value:.6f}," for key, value in step_metrics.items())
                        print(f"{time_str()} [EPOCH{epoch+1:04d}] [VAL] [{epoch_val_patches:06d}/{self.val_patches:06d}], loss: {step_loss: 6f}, {metric_str}")
            # calculate epoch loss and metrics
            ...
            print(f"{time_str()} [EPOCH{epoch+1:04d}] [VAL] loss: {epoch_val_loss:6f}, {metric_str}")
            # log_writer
            self.log_writer.add_scalars("Loss", {"train_loss": epoch_train_loss, "val_loss": epoch_val_loss}, epoch)
            for key in epoch_train_metrics.keys():
                self.log_writer.add_scalars(f"{key} channel", {
                    "train_metric": epoch_train_metrics[key], "val_metric": epoch_val_metrics[key]}, epoch)
            # update best_loss and patience
            if epoch_val_loss < best_val_loss:
                metric_str = " ".join(f"{key} channel: {value:.6f}," for key, value in epoch_val_metrics.items())
                print(f"{time_str()} Model improved from {best_val_loss:.6f} to {epoch_val_loss:.6f}, {metric_str}")
                epochs_without_improvement = 0
                ...
                print(f"{time_str()} no improvement, Patience: {epochs_without_improvement:04d}/{self.patience:04d}")
                if epochs_without_improvement >= self.patience:
                    metric_str = " ".join(f"{key} channel: {value:.6f}," for key, value in best_val_metrics.items())
                    print(f"{time_str()} Out of patience, stopping training. Best validation loss: {best_val_loss:.6f}")
                    return
    ...
    def run(self):
        if self.cfg.TRAIN.ENABLE:
            print(f"{time_str()} start instance segmentation training")
            self.train()
        elif self.cfg.INFER.ENABLE:
            print(f"{time_str()} start instance segmentation inference")
            self.infer()
