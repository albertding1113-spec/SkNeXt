...
def sha256_file(path: str | Path, block_size: int = 64 * 1024 * 1024) -> str:
    path = Path(path)
    h = hashlib.sha256()
...
def repair_file_by_reference(
    bad_file: str | Path,
    reference_file: str | Path,
    output_file: str | Path,
    diff_log_csv: str | Path | None = None,
    block_size: int = 64 * 1024 * 1024,
) -> dict:
    """
    使用 reference_file 修复 bad_file 中发生字节变化的位置。

    参数
    ----
    bad_file:
        损坏的文件，比如微信传输后的 .vsseg
    ...
    bad_file = Path(bad_file)
    reference_file = Path(reference_file)
    output_file = Path(output_file)

    if diff_log_csv is not None:
        diff_log_csv = Path(diff_log_csv)

    if not bad_file.exists():
        raise FileNotFoundError(f"bad_file not found: {bad_file}")

    if not reference_file.exists():
        raise FileNotFoundError(f"reference_file not found: {reference_file}")
    ...
    try:
        if diff_log_csv is not None:
            ...
            log_f = diff_log_csv.open("w", newline="", encoding="utf-8")
            writer = csv.writer(log_f)
            writer.writerow(["offset", "bad_hex", "reference_hex"])

        with bad_file.open("rb") as f_bad, \
             reference_file.open("rb") as f_ref, \
             output_file.open("wb") as f_out:

            while True:
                bad_block = f_bad.read(block_size)
                ref_block = f_ref.read(block_size)

                if not bad_block and not ref_block:
                    break

                n_total_blocks += 1

                # 如果坏文件缺失尾部，则直接补参考文件尾部
                if not bad_block and ref_block:
                    f_out.write(ref_block)
                    n_diff_blocks += 1
                    n_diff_bytes += len(ref_block)

                    if writer is not None:
                        for i, ref_byte in enumerate(ref_block):
                            writer.writerow([global_offset + i, "MISSING", f"{ref_byte:02x}"])

                    global_offset += len(ref_block)
                    continue

                # 如果坏文件比参考文件更长，则多余部分丢弃
                if bad_block and not ref_block:
                    n_diff_blocks += 1
                    n_diff_bytes += len(bad_block)

                    if writer is not None:
                        for i, bad_byte in enumerate(bad_block):
                            writer.writerow([global_offset + i, f"{bad_byte:02x}", "EXTRA"])

                    global_offset += len(bad_block)
                    continue
                ...
                if bad_block == ref_block:
                    f_out.write(bad_block)
                    global_offset += len(bad_block)
                    continue
                ...
                # 长度可能不同，按参考文件长度输出
                fixed = bytearray(ref_block)

                min_len = min(len(bad_block), len(ref_block))
                for i in range(min_len):
                    if bad_block[i] != ref_block[i]:
                        n_diff_bytes += 1
                        if writer is not None:
                            writer.writerow([
                                global_offset + i,
                ...
                if len(bad_block) != len(ref_block):
                    n_diff_bytes += abs(len(bad_block) - len(ref_block))

                    if writer is not None:
                        if len(bad_block) < len(ref_block):
                            for i in range(len(bad_block), len(ref_block)):
                                writer.writerow([
                                    global_offset + i,
                                    "MISSING",
                                    f"{ref_block[i]:02x}",
                            ...
                            for i in range(len(ref_block), len(bad_block)):
                                writer.writerow([
                                    global_offset + i,
                                    f"{bad_block[i]:02x}",
                                    "EXTRA",
                ...
                f_out.write(fixed)
                global_offset += max(len(bad_block), len(ref_block))

    finally:
        if log_f is not None:
            log_f.close()

    ref_hash = sha256_file(reference_file)
    out_hash = sha256_file(output_file)
    bad_hash = sha256_file(bad_file)

    success = ref_hash == out_hash
    ...
    return {
        ...
        "different_blocks": n_diff_blocks,
        "different_bytes": n_diff_bytes,
        "bad_sha256": bad_hash,
...
def main():
    parser = argparse.ArgumentParser(
        description="Repair a corrupted .vsseg file using a correct reference file."
    )
    ...
    parser.add_argument(
        "--ref",
        required=True,
        help="正确的原始/备份 .vsseg 文件路径",
    )
    parser.add_argument(
        "--out",
        required=True,
        help="修复后的输出 .vsseg 文件路径",
    )
    parser.add_argument(
        "--log",
        default=None,
        help="可选：保存字节差异日志 CSV",
    )
    parser.add_argument(
        "--block-size-mb",
        type=int,
        default=64,
        help="分块大小，单位 MB，默认 64",
    )

    args = parser.parse_args()

    result = repair_file_by_reference(
        bad_file=args.bad,
        reference_file=args.ref,
        output_file=args.out,
        diff_log_csv=args.log,
    ...
    if result["success"]:
        print("\n修复成功：输出文件 SHA-256 与参考文件完全一致。")
    else:
        print("\n修复失败：输出文件 SHA-256 与参考文件不一致，请检查参考文件或磁盘写入。")
